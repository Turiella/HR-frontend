    ...
    <div className="max-w-5xl px-4 py-8 mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => navigate(-1)} className="px-3 py-1 text-sm text-white bg-white/10 border border-white/20 rounded hover:bg-white/20">Volver</button>
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <div className="flex flex-wrap gap-2 mt-2 text-xs">
              <span className="px-2 py-0.5 bg-white/10 border border-white/20 rounded">Género: <span className="font-medium">{user?.gender || '—'}</span></span>
              <span className="px-2 py-0.5 bg-white/10 border border-white/20 rounded">Ciudad: <span className="font-medium">{user?.city || '—'}</span></span>
              {user?.created_at && (
                <span className="px-2 py-0.5 bg-white/10 border border-white/20 rounded">Alta: {new Date(user.created_at).toLocaleDateString()}</span>
              )}

      {/* CV recomendado (si hay filtros en la URL) */}
      {recommendedCv?.cv && (
        <div className="p-4 mt-6 bg-indigo-500/10 border border-indigo-500/30 rounded-lg">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <div className="font-medium text-gray-100 flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 text-[10px] rounded bg-white/10 border border-white/20">{String(recommendedCv.cv.category || '')}</span>
                {recommendedCv.cv.version != null && (
                  <span className="px-2 py-0.5 text-[10px] rounded bg-white/10 border border-white/20">v{recommendedCv.cv.version}</span>
                )}
              </div>
              <div className="mt-1 text-xs text-gray-300">
                Score: <span className="font-semibold">{Number(recommendedCv.cv.classification_score ?? 0).toFixed(2)}</span>
                {' '}• Required: {recommendedCv.reasons?.requiredMatches ?? 0}
                {' '}• Preferred: {recommendedCv.reasons?.preferredMatches ?? 0}
                {' '}• JD: {recommendedCv.reasons?.jdMatches ?? 0}
              </div>
            </div>
            <div className="w-full max-w-xs">
              ...
              <div className="h-2 overflow-hidden bg-black/30 rounded">
                <div className="h-full bg-gradient-to-r from-indigo-500 to-violet-500" style={{ width: `${Math.min(100, ((Number(recommendedCv.cv.classification_score ?? 0) + (recommendedCv.reasons?.requiredMatches ?? 0) + (recommendedCv.reasons?.preferredMatches ?? 0)) * 10))}%` }} />
              </div>
            </div>
          </div>
        </div>
          ...
          {hasCoords && (
            <div className="w-full sm:w-64 h-40 overflow-hidden rounded border border-white/10">
              <iframe
                title="Mapa"
                className="w-full h-full"
                src={`https://www.openstreetmap.org/export/embed.html?bbox=${user.lon-0.02}%2C${user.lat-0.02}%2C${user.lon+0.02}%2C${user.lat+0.02}&layer=mapnik&marker=${user.lat}%2C${user.lon}`}
              />
            </div>
          )}
        </div>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {catList.map(cat => (
          <button
            ...
            className={`px-3 py-1 text-sm rounded border ${activeCat === cat.name ? 'bg-indigo-600 text-white border-indigo-500' : 'bg-white/5 border-white/10 text-gray-200'}`}
          >
            {cat.name.toUpperCase()}
          </button>
        ))}
        {catList.length === 0 && <span className="text-sm text-gray-400">Sin categorías aún</span>}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="w-full max-w-3xl p-4 bg-neutral-900 border border-white/10 rounded-lg">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => setOpenJsonIndex(null)} className="px-2 py-1 text-xs bg-white/10 border border-white/20 rounded">Cerrar</button>
            </div>
